...
- News: decide if one item is GENUINELY relevant to her point. If so, reference it naturally in one or two sentences, naming the publication. You have ONLY the headline, not the article: say that the piece exists and what topic it covers (e.g. "Cosmetics Business ran a piece this month on how personal care products are measured"), never what it argues, highlights, finds or recommends. If the headline is a question, do not answer it on the article's behalf; set news_index to its number and write a one-line news_summary restating the headline. If none fits naturally, ignore them all: news_index = -1 and news_summary = "". Never force it.
